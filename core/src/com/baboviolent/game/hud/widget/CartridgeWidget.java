 ...
 * @author Nathan Sweet */
public class CartridgeWidget extends Widget {
	static private final Color tempColor = new Color();

	private CartridgeWidgetStyle style;
	...
	private float textOffsetX = -40;
	private float textOffsetY = 0;

	public CartridgeWidget (CharSequence text, Skin skin) {
		this(text, skin.get(CartridgeWidgetStyle.class));
	}

	public CartridgeWidget (CharSequence text, Skin skin, String styleName) {
		this(text, skin.get(styleName, CartridgeWidgetStyle.class));
	}

	public CartridgeWidget (CharSequence text, CartridgeWidgetStyle style) {
		if (text != null) this.text.append(text);
		setStyle(style);
		setSize(getPrefWidth(), getPrefHeight());
	}

	public void setStyle (CartridgeWidgetStyle style) {
	...
	/** Returns the label's style. Modifying the returned style may not have an effect until {@link #setStyle(CartridgeWidgetStyle)} is
	 * called. */
	public CartridgeWidgetStyle getStyle () {
		return style;
	}
	...
	public void draw (Batch batch, float parentAlpha) {
		...
		cache.tint(color);
		cache.setPosition(getX()+textOffsetX, getY()+textOffsetY);
		cache.draw(batch);
	}
	...
	/** The style for a label, see {@link CartridgeWidget}.
	 * @author Nathan Sweet */
	static public class CartridgeWidgetStyle {
		...
		public CartridgeWidgetStyle () {
		}

		public CartridgeWidgetStyle (BitmapFont font, Color fontColor, Drawable background) {
			this.font = font;
			this.fontColor = fontColor;
			this.background = background;
		}

		public CartridgeWidgetStyle (CartridgeWidgetStyle style) {
